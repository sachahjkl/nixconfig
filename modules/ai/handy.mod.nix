{self, ...}: {
  perSystem = {pkgs, ...}: let
    ...
    handyIcon = pkgs.runCommand "handy-icon" {} ''
      ...
      cp ${pkgs.fetchurl {
        url = "https://raw.githubusercontent.com/cjpais/Handy/v${version}/src-tauri/icons/128x128.png";
    ...
    managedSettingsFile = pkgs.writeText "handy-managed-settings.json" (builtins.toJSON managedSettings);
    ...
    handyPkg = pkgs.symlinkJoin {
      name = "handy-${version}";
      paths = [handyAppImage handyDesktop handyIcon];
      ...
      postBuild = ''
        ...
        if [ ! -e "$settings_store" ]; then
          ...
          ${pkgs.lib.getExe pkgs.jq} --slurpfile managed ${managedSettingsFile} \
            '.settings = (.settings * $managed[0])' \
            "$settings_store" > "$settings_store.tmp"
          mv "$settings_store.tmp" "$settings_store"
        fi
        chmod 600 "$settings_store"
  ...
  flake.nixosModules.handy = {
    ...
    cfg = config.ai.handy;
  in {
    options.ai.handy.enable = lib.mkEnableOption "Handy offline speech-to-text";
    ...
    config = lib.mkIf cfg.enable {
      ...
      systemd.user.services.handy = {
        ...
        serviceConfig = {
          ExecStart = "${lib.getExe' selfPkgs.handy "handy"} --start-hidden";
          Restart = "on-failure";
          RestartSec = 2;
        };
      };
