    ...
    in
    {
      hjem.users.${userName}.xdg.config.files = {
        "waybar/config".text = builtins.toJSON {
          ...
          "custom/power-button" = { format = u "F011"; on-click = "${lib.getExe rofiPkg} -show power-menu -modi 'power-menu:rofi-power-menu --choices=${powerChoices}'"; tooltip-format = "Power menu"; };
          cpu = { interval = 5; format = "${u "F2DB"}  {usage}%"; max-length = 10; };
          temperature = { hwmon-path-abs = hyprCfg.waybar.temperature.hwmonPath; input-filename = hyprCfg.waybar.temperature.inputFilename; critical-threshold = 75; tooltip = false; format-critical = "({temperatureC}°C)"; format = "({temperatureC}°C)"; };
          disk = { interval = 30; format = "${u "F0C7"} {percentage_used}%"; path = "/"; tooltip = true; unit = "GB"; tooltip-format = "Available {free} of {total}"; };
          memory = { interval = 10; format = "${u "F0E7"}  {percentage}%"; max-length = 10; tooltip = true; tooltip-format = "RAM - {used:0.1f}GiB used"; };
          "wlr/taskbar" = { format = "{icon} {title:.17}"; icon-size = 28; spacing = 3; on-click-middle = "close"; tooltip-format = "{title}"; ignore-list = [ ]; on-click = "activate"; };
          tray = { icon-size = 18; spacing = 3; };
          clock = { format = "      {:%R\n %d.%m.%Y}"; tooltip-format = "<tt><small>{calendar}</small></tt>"; calendar = { mode = "year"; mode-mon-col = 3; weeks-pos = "right"; on-scroll = 1; on-click-right = "mode"; format = { months = "<span color='#ffead3'><b>{}</b></span>"; days = "<span color='#ecc6d9'><b>{}</b></span>"; weeks = "<span color='#99ffdd'><b>W{}</b></span>"; weekdays = "<span color='#ffcc66'><b>{}</b></span>"; today = "<span color='#ff6699'><b><u>{}</u></b></span>"; }; }; actions = { on-click-right = "mode"; on-click-forward = "tz_up"; on-click-backward = "tz_down"; on-scroll-up = "shift_up"; on-scroll-down = "shift_down"; }; };
          network = { format-wifi = " {icon}"; format-ethernet = " ${u "EB01"} "; format-disconnected = u "F0719"; format-icons = [ "${u "F1EB"} " "${u "F1DF"} " "${u "F1E2"} " "${u "F1E2"} " "${u "F1E8"} " ]; };
          battery = { states = { good = 95; warning = 30; critical = 20; }; format = "{icon} {capacity}%"; format-charging = "${u "F1E6"} {capacity}%"; format-plugged = "${u "F1E6"} {capacity}%"; format-alt = "{time} {icon}"; format-icons = [ (u "F008E") (u "F007A") (u "F007B") (u "F007C") (u "F007D") (u "F007E") (u "F007F") (u "F0080") (u "F0081") (u "F0082") (u "F0079") ]; };
          pipewire = { max-volume = 150; scroll-step = 10; format = "{icon}"; tooltip-format = "{volume}%"; format-muted = "${u "F026"} "; format-icons.default = [ "${u "F026"} " "${u "F027"} " "${u "F028"} " ]; on-click = "pwvucontrol"; };
        };
        ...
        "waybar/style.css".text = ''
          ...
          * { text-shadow: none; box-shadow: none; border: none; border-radius: 0; font-family: "${config.preferences.theme.fonts.sans}", "Noto Sans"; font-weight: 600; font-size: 12.7px; }
          ...
          #cpu, #disk, #memory { padding: 3px; }
          #temperature { color: transparent; font-size: 0px; transition: all 0.25s cubic-bezier(0.165, 0.84, 0.44, 1); }
          #temperature.critical { padding-right: 3px; color: orange; font-size: initial; border-bottom: 3px dashed orange; transition: all 0.25s cubic-bezier(0.165, 0.84, 0.44, 1); }
          #window { border-radius: 10px; margin-left: 20px; margin-right: 20px; }
          ...
          #clock { padding-right: 5px; padding-left: 5px; transition: all 0.25s cubic-bezier(0.165, 0.84, 0.44, 1); }
