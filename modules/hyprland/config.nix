...
lib.mkIf isHypr {
  ...
  home-manager.users.${userName} = {
    home.pointerCursor = {
      ...
      name = config.sacha.theme.cursor;
      size = config.sacha.theme.cursorSize;
    };

    gtk = {
      enable = true;
      theme = { name = config.sacha.theme.gtkTheme; package = pkgs.arc-theme; };
      iconTheme = { name = config.sacha.theme.iconTheme; package = pkgs.papirus-icon-theme; };
      font = { name = config.sacha.theme.fonts.sans; size = 11; };
    };
    ...
    wayland.windowManager.hyprland = {
      ...
      settings = {
        ...
        exec-once = [
          ...
          "hyprpaper"
          "hypridle"
          "[workspace special:magic silent; float; size 1200 1000; move 80 80] $terminal --title Scratchpad"
        ];
    ...
    dconf = {
      enable = true;
      settings."org/gnome/desktop/interface".text-scaling-factor = 1.0;
    };
  };
}
